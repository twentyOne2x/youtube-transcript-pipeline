...
from src.index_youtube.constants import KEYWORDS_TO_INCLUDE, KEYWORDS_TO_EXCLUDE, YOUTUBE_VIDEOS_CSV_FILE_PATH, AUTHORS, \
    FIRMS
...
async def get_multiple_video_details(channel_name, youtube, video_ids, keywords, keywords_to_exclude, PASSTHROUGH):
    ...
    async def process_id_batch(id_batch):
        try:
            ...
            # Check if 'video_id' column exists for backward compatibility
            if 'video_id' in youtube_videos_df.columns:
                existing_video_ids = set(youtube_videos_df['video_id'].astype(str))
            else:
            ...
            for item in items:
                ...
                # Check if video already exists by ID (primary check) or title (fallback)
                video_exists = video_id in existing_video_ids
                if not video_exists and 'title' in youtube_videos_df.columns:
                    video_row = youtube_videos_df[youtube_videos_df['title'] == video_title]
                    video_exists = not video_row.empty

                def append_video_details(video_exists, video_info_item, video_title, video_id):
                    if video_exists:
                        # print(f"Video {video_id} - {video_title} already exists in the CSV file. Skipping...")
                        return None
                    ...
                    return {
                        'video_id': video_id,  # Add video_id as first field
                        'title': video_title,
                ...
                if video_info_item['channelTitle'] in PASSTHROUGH:
                    video_detail = append_video_details(video_exists, video_info_item, video_title, video_id)
                    if video_detail:
...
async def get_video_info(session, credentials: ServiceAccountCredentials, api_key: str, channel_id: str,
                         channel_name: str, PASSTHROUGH, max_results: int = 50) -> List[dict]:
    """
    ...
    while True:
        ...
        video_details = await get_multiple_video_details(channel_name, youtube, video_ids, KEYWORDS_TO_INCLUDE,
                                                         KEYWORDS_TO_EXCLUDE, PASSTHROUGH)
...
def save_video_info_to_csv(video_info_list, csv_file_path, existing_video_ids, headers):
    # Remove duplicates based on video IDs (primary) and titles (secondary)
    video_info_list = [
        video_info for video_info in video_info_list
        if video_info.get('video_id') not in existing_video_ids
    ]
    ...
    # Filter out existing videos by video_id
    new_videos = [video for video in video_info_list if video.get('video_id') not in existing_video_ids]
...
async def fetch_and_save_channel_videos_async(session, channel_id, channel_name, credentials, api_key, csv_file_path,
                                              existing_video_ids, headers, PASSTHROUGH):
    video_info_list = await get_video_info(session, credentials, api_key, channel_id, channel_name, PASSTHROUGH)

    save_video_info_to_csv(video_info_list, csv_file_path, existing_video_ids, headers)
    logging.info(f"[{channel_name}] Saved {len(video_info_list)} videos to CSV file {csv_file_path}.")


async def fetch_youtube_videos(api_key, yt_channels, yt_playlists, keywords, keywords_to_exclude, PASSTHROUGH,
                               fetch_videos):
    """
    ...
    channels_in_csv, channels_not_in_csv = separate_channels_based_on_csv(channel_handle_to_name,
                                                                          existing_channel_names, yt_channels)

    if fetch_videos:
        await fetch_channel_videos(api_key, channel_handle_to_name, channels_in_csv, channels_not_in_csv, credentials,
                                   csv_file_path, existing_video_ids, headers, yt_channels, PASSTHROUGH)

        await fetch_playlist_videos(api_key, credentials, csv_file_path, existing_video_ids, headers, yt_playlists)

    return existing_data


async def fetch_playlist_videos(api_key, credentials, csv_file_path, existing_video_ids, headers, yt_playlists):
    if yt_playlists:
        for playlist_id in yt_playlists:
            video_info_list = get_videos_from_playlist(credentials, api_key, playlist_id)
            # Ensure video_info_list items have video_id field
            for video in video_info_list:
                if 'url' in video and 'video_id' not in video:
                    # Extract video_id from URL if not present
                    url = video['url']
                    if 'v=' in url:
                        video['video_id'] = url.split('v=')[1].split('&')[0]
                    elif 'youtu.be/' in url:
                        video['video_id'] = url.split('youtu.be/')[1].split('?')[0]
            save_video_info_to_csv(video_info_list, csv_file_path, existing_video_ids, headers)


async def fetch_channel_videos(api_key, channel_handle_to_name, channels_in_csv, channels_not_in_csv, credentials,
                               csv_file_path, existing_video_ids, headers, yt_channels, PASSTHROUGH):
        ...
        for channel_handle in all_channels:
            ...
            if channel_name:
                ...
                if channel_id:
                    ...
                    # session, channel_id, channel_name, credentials, api_key, csv_file_path, existing_video_ids, headers
                    await fetch_and_save_channel_videos_async(session, channel_id, channel_name, credentials, api_key,
                                                              csv_file_path, existing_video_ids, headers, PASSTHROUGH)
...
def load_existing_data(csv_file_exists, csv_file_path):
    # Create a DataFrame to store existing data read from the CSV file and create sets of existing video IDs and channel names
    if csv_file_exists:
        existing_data_df = pd.read_csv(csv_file_path, encoding='utf-8')

        # Create a set for existing video IDs for faster lookup (primary identifier)
...
def setup_csv():
    ...
    # Updated headers with video_id as first column
    ...
    if csv_file_exists:
        ...
        # Check if we need to migrate old format to new format
        if 'video_id' not in existing_headers:
            logging.info("Migrating CSV to include video_id column...")
            # Read full dataframe
            ...
            logging.info(f"Migration complete. Added video_id column to {len(df)} rows.")

        elif existing_headers != headers:
            # Create a new CSV file with the specified headers if headers don't match
...
def drop_duplicates_and_write_to_csv(filtered_away_csv_file_path, filtered_away_video_info_list, headers):
    ...
    # Combine the existing rows with the new rows, and remove duplicates based on video_id (primary) or url (fallback)
    all_rows = existing_rows + filtered_away_video_info_list

    # Deduplicate by video_id if present, otherwise by url
    seen_ids = set()
    deduplicated_rows = []
    for row in all_rows:
...
def filter_and_remove_videos(input_csv_path, keywords, keywords_to_exclude, PASSTHROUGH, channel_specific_filters=None):
    ...
    if keywords:  # Only filter if keywords exist
        global_filtered_df = non_passthrough_df[
            non_passthrough_df['title'].str.lower().str.contains('|'.join(keywords).lower(), na=False)]
    else:
    ...
    if keywords_to_exclude:  # Only filter if exclusion keywords exist
        passthrough_df = passthrough_df[
            ~passthrough_df['title'].str.lower().str.contains('|'.join(keywords_to_exclude).lower(), na=False)]
    ...
    channels_with_specific_filters = set(global_filtered_df['channel_name'].str.lower()) & set(
        channel_specific_filters.keys())
    ...
    for channel in channels_with_specific_filters:
        ...
        channel_filtered_df = channel_df[
            channel_df['title'].str.lower().str.contains('|'.join(channel_keywords).lower(), na=False)]
    ...
    final_filtered_df = pd.concat([final_filtered_df, global_filtered_df[
        ~global_filtered_df['channel_name'].str.lower().isin(channels_with_specific_filters)]])

    # Identify removed videos using video_id as primary identifier
    if 'video_id' in df.columns and 'video_id' in final_filtered_df.columns:
        removed_df = df[~df['video_id'].isin(final_filtered_df['video_id'])]
    else:
    ...
    for _, removed_video in removed_df.iterrows():
        video_id = removed_video.get('video_id', 'NO_ID')
        logging.info(
            f"Removed video: {video_id} - {removed_video['title']} - Channel: [{removed_video['channel_name']}]")
...
async def fetch_all_videos(api_key: str, yt_channels: Optional[List[str]] = None,
                           yt_playlists: Optional[List[str]] = None,
                           keywords: List[str] = None, keywords_to_exclude: List[str] = None,
                           PASSTHROUGH: List[str] = None, fetch_videos: bool = True):
    await fetch_youtube_videos(api_key, yt_channels, yt_playlists, keywords, keywords_to_exclude, PASSTHROUGH,
                               fetch_videos)
...
def run():
    ...
    # Drop duplicates based on video_id if it exists, otherwise on all columns
